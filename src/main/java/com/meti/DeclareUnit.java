...
public class DeclareUnit implements Unit {
    ...
    public String compile(String trim, Compiler compiler) {
        ...
        declarations.pop();
        if (name.equals("main") || (compiledValue.isBlank() && flags.contains(Flag.NATIVE)) || renderType.endsWith(")")) {
            return "";
        } else {
			String s = callback.toString();
			callback.replace(0, callback.length(), "");
			return s + renderType + " " + name + "=" + compiledValue + ";";
        }
    }
