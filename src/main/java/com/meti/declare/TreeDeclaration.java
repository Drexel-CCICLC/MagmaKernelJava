...
import com.meti.node.bracket.struct.*;
...
public class TreeDeclaration implements Declaration {
	...
	TreeDeclaration(Type type, boolean parameter, Declarations declarations,
	                List<String> stack, List<Declaration> children, Set<Flag> flags) {
		this.type = type;
	...
	public Node toStruct(List<? extends Parameter> parameters) {
		Map<String, Type> map = parameters.stream()
				.collect(Collectors.toMap(
						(Function<Parameter, String>) Parameter::name,
						(Function<Parameter, Type>) Parameter::type));
		return new StructNode(name(), map);
	}
