...
public class StructParser implements Parser {
	...
	public Optional<Node> parse(String value, Compiler compiler) {
		...
		if (trim.startsWith("(")) {
			...
			Map<String, Type> children = new HashMap<>(Arrays.stream(paramsString.split(","))
					.map(String::trim)
			...
			children.forEach(declarations::define);
			...
			functions.add(new StructNode(returnType, declarations.current().name(), children, block));
